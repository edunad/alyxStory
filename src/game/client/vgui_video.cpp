...
m_bStarted(false)
{
	vgui::VPANEL pParent = enginevgui->GetPanel(PANEL_INGAMESCREENS);
	SetParent(pParent);
	SetVisible(false);
	...
	SetScheme(vgui::scheme()->LoadSchemeFromFile("resource/xl_videopanelscheme.res", "xl_videopanelscheme"));
	LoadControlSettings("resource/UI/xl_videopanel.res");
...
bool VideoPanel::BeginPlayback(const char *pFilename)
{
	...
	m_VideoMaterial = g_pVideo->CreateVideoMaterial("VideoBIKMaterial", pFilename, "GAME",
		VideoPlaybackFlags::DEFAULT_MATERIAL_OPTIONS | VideoPlaybackFlags::DEFAULT_FULLSCREEN_OPTIONS,
	...
	int nWidth, nHeight;
	m_VideoMaterial->SetLooping(m_bLooping);
...
void VideoPanel::DoModal(void)
{
	...
	DevMsg("Video panel constructed\n");
}
	...
	bool shouldLoop)
{
	...
	pVideoPanel->m_bLooping = shouldLoop;
...
//-----------------------------------------------------------------------------
void CreateVideoPanel(const char *lpszFilename, const char *lpszExitCommand, int nWidth, int nHeight,bool bShouldloop, bool bAllowInterruption)
{
...
CON_COMMAND(playvideo_nointerrupt, "Plays a video without ability to skip: <filename> [width height]")
{
	...
	CreateVideoPanel(args[1], NULL, nScreenWidth, nScreenHeight,false, false);
}
...
CON_COMMAND(playvideo_exitcommand, "Plays a video and fires and exit command when it is stopped or finishes: <filename> <exit command>")
{
	...
	char *pExitCommand = Q_strstr(args.GetCommandString(), args[5]);
	bool isLoop = (Q_atoi(args[2]) == 1);

	CreateVideoPanel(args[1], pExitCommand, 0, 0, isLoop, true);
}
...
CON_COMMAND(playvideo_exitcommand_nointerrupt, "Plays a video (without interruption) and fires and exit command when it is stopped or finishes: <filename> <exit command>")
{
	...
	CreateVideoPanel(args[1], pExitCommand, 0, 0, isLoop, false);
}
