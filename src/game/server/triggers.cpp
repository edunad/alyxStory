...
void CTriggerCamera::Enable(void)
{
	...
	{
		UTIL_SetOrigin(this, m_hPlayer->EyePosition());
		SetLocalAngles(QAngle(m_hPlayer->GetLocalAngles().x, m_hPlayer->GetLocalAngles().y, m_hPlayer->GetLocalAngles().z));
...
void CTriggerCamera::CameraThink()
{
	...
	// Check if camera contains a path to follow
	if (m_pPath)
		Move();

	// Check if camera a target or if the player can move the camera
	if (m_hTarget || HasSpawnFlags(SF_CAMERA_PLAYER_LOOK))
		FollowTarget();
	
	// TODO : Apply any shakes
	/*
	if (m_hPlayer){
		CBasePlayer *pPlayer = ToBasePlayer(m_hPlayer);
		SetLocalOrigin(pPlayer->EyePosition());
	}
	*/
